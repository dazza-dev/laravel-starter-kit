...
Queues run on Redis through Horizon. Two supervisors, one queue each, so a long job never delays a
short one: `supervisor-fast` (`default`, `mail`, `notifications`) and `supervisor-heavy` (`reports`).
...
On the frontend, consume these via `useOptions()` in `vue-starter-kit` (`src/core/composables/useOptions.ts`).
...
- `nestjs-starter-kit` — the same API in NestJS; both are interchangeable
